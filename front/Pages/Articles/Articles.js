(() => {
    ...
    if (!("trashed" in window.tsm) || !("ids" in window.tsm) || !window.tsm.trashed) {
        return;
    }
    const ids = window.tsm.ids.split(",");
    if (ids.length > 1) {
    ...
    async function deleteTranslations(modal, id) {
        ...
        const fetchResponse = await fetch(`${tsm.url}article_deleted_delete_translations&post_id=${id}`);
    ...
    async function loadModalTraduireSansMigraineForTrash(id) {
        const fetchResponse = await fetch(`${tsm.url}article_deleted_render&post_id=${id}`);
